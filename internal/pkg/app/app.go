...
func GrpcServerOption(svr *rpc.Server) Option {
	return func(app *Application) error {
		//svr.Application(app.name)
		//app.grpcServer = svr
		return nil
...
func LauncherOption(launcher Launcher) Option {
	return func(app *Application) error {
...
func (a *Application) Start() error {
	...
	if a.launcher != nil {
		if err := a.launcher.Start(); err != nil {
			return errors.Wrap(err, "launcher start error")
		}
	}
...
func (a *Application) AwaitSignal() {
	...
	case s := <-c:
		...
		if a.launcher != nil {
			if err := a.launcher.Stop(); err != nil {
				a.logger.Warn("stop launcher error", zap.Error(err))
			}
		}
